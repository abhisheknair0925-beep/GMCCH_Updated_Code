...
$crnumber=$_POST['crnumber'];
    ...
    echo $inst_adds="INSERT INTO tbl_user(tbl_user_crno,tbl_user_name,tbl_user_age,tbl_user_gender)VALUES('$crnumber','$name','$age','$gender')";
	$add_ins=mysqli_query($dbconnect,$inst_adds);
	if($add_ins==1)
	{
		echo "<script>alert('Successfully saved users')</script>";
		echo "<script>window.location.href='Add_users.php'</script>";
	}
	else
	{
